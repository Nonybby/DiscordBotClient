...
class GlobalConfig {
	...
	loadConfig(iniString) {
		...
		try {
			...
			console.error(
				'Failed to parse configuration, reverting to old config',
				oldData,
			);
			this.config = oldData;
			throw e;
	...
	validateConfig() {
		for (const key in this.config) {
			if (!this.defaultConfig().hasOwnProperty(key)) {
				console.warn(
					`Unknown configuration key "${key}" found, removing it.`,
				);
		...
		if (typeof this.config.cache_assets !== 'boolean') {
			throw new Error(
				'Invalid value for cache_assets, expected boolean.',
			);
		}
		if (
			typeof this.config.guilds_per_shard !== 'number' ||
			this.config.guilds_per_shard <= 0
		) {
			throw new Error(
				'Invalid value for guilds_per_shard, expected positive number.',
			);
		}
		if (typeof this.config.suppress_intent_warning !== 'boolean') {
			throw new Error(
				'Invalid value for suppress_intent_warning, expected boolean.',
			);
		}
		if (typeof this.config.generate_fake_profile !== 'boolean') {
			throw new Error(
				'Invalid value for generate_fake_profile, expected boolean.',
			);
		}
	}
	parseIniValue(val) {
...
**Default:** \`true\``,
				enum: {
					true: {
						documentation:
							'Enable fake profile data injection (default).',
					},
					false: {
						documentation:
							'Disable fake profile data injection, using only real data from Discord.',
					},
				},
			},
		};
